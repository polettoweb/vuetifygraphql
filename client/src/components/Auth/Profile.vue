          ...
          <v-flex xs7>
            <v-card-title primary-title>
              <div>
                ...
                <div class="body-2">Joined {{formatJoinDate(user.joinDate)}}</div>
                <div
    ...
    <v-container class="mt-3" v-else>
      <v-flex xs12>
        <h2 class="font-weight-light">
          Your posts
          <span class="font-weight-regulart">({{userPosts.length}})</span>
        </h2>
      </v-flex>
      <v-layout row wrap>
        <v-flex xs12 sm6 v-for="post in userPosts" :key="post._id">
          <v-card class="mt-3 ml-1 mr-2 pt-2" hover>
            <v-btn @click="loadPost(post)" color="info" floating fab small dark class="mb-2 mr-2">
              <v-icon>edit</v-icon>
            </v-btn>
            <v-btn
      ...
      <v-card>
        ...
        <v-container class="px-10">
          ...
          >
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn type="submit" :disabled="!isFormValid" class="success--text" text>Update</v-btn>
